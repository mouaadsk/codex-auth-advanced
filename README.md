...
The Grok 4.5 and Grok 4.6 entries intentionally omit `context_window`. VSLLM's model catalog does not currently publish a verified limit for those IDs, so Grok Build applies its conservative custom-model default (currently 200,000 tokens) instead of delaying compaction based on an unverified one-million-token claim. Existing sessions retain their accumulated history; compact them explicitly or start a fresh session after reconfiguring Grok Build.
...
If every same-account compaction attempt on the active account fails, the proxy performs a last-resort cross-account fallback: it tries the same model on every other usable same-group API-key account in registry order, running the full `/v1/responses` then `/v1/chat/completions` sequence with the same per-account retry/watchdog budgets. The first account to return a usable summary wins, and the active account is never mutated by the fallback. Pinned proxy URLs (e.g. `accounts/<key>/v1`) stay on the caller-selected account and are exempt from this fallback. The proxy returns 502 only after the active account and every other usable same-group account has failed.

For provider/account/channel transitions that cannot decrypt earlier reasoning state, the VSLLM and LLMAPI templates enable a narrow compatibility repair. If `/responses/compact` explicitly rejects `encrypted_content` as invalid or unverifiable, the proxy retries that compact request once on the same account and endpoint using only readable user/assistant text. If an ordinary `/responses` request receives the same HTTP 400/422 validation error before any output, the proxy retries once on the same account and endpoint after removing opaque encrypted reasoning state while preserving readable messages and tool history. It does not proactively strip encryption, replay successful/partially streamed requests, or apply this behavior to providers that have not opted in.
