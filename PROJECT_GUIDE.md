...
VSLLM and LLMAPI enable the encrypted-content compatibility repair. When `/responses/compact` explicitly returns an invalid/unverifiable encrypted-content validation error, the proxy retries once on the same account and endpoint with a plaintext-only user/assistant transcript. When an ordinary `/responses` turn receives the same pre-output HTTP 400/422 validation error, the proxy retries once on the same account and endpoint after removing opaque reasoning/encrypted-content state while preserving readable messages and tool-call/tool-output history. Successful or partially streamed turns are never replayed, and providers that did not opt into the template flag retain the original request.
